...
namespace VocaNerd.EditorTools
{
    ...
    {
        private const string DefaultBuildPath = "Builds/Mac/VocaNerd.app";
        private const string VideoSourceDir = "Assets/Video";
        private const string StreamingAssetsRelativeDir = "Contents/Resources/Data/StreamingAssets";
        private static readonly string[] SafeBuildRootNames = { "Build", "Builds", "build", "builds" };

        [MenuItem("VocaNerd/Build/Mac")]
        public static void Build()
        {
            ...
            if (scenes.Length == 0)
                throw new InvalidOperationException("[MacBuildScript] No enabled scenes found in EditorBuildSettings.");
            ...
            Debug.Log("[MacBuildScript] Switching active build target to macOS.");
            if (!EditorUserBuildSettings.SwitchActiveBuildTarget(BuildTargetGroup.Standalone, BuildTarget.StandaloneOSX))
                throw new InvalidOperationException("[MacBuildScript] Failed to switch build target to macOS. Make sure the macOS build support module is installed.");
            ...
            Debug.Log($"[MacBuildScript] Result: {summary.result}, size: {summary.totalSize} bytes, time: {summary.totalTime}.");

            if (summary.result != BuildResult.Succeeded)
                throw new InvalidOperationException($"[MacBuildScript] macOS build failed: {summary.result}");
        ...
        private static void CleanBuildPath(string buildPath)
        {
            if (!IsSafeBuildPath(buildPath))
                throw new InvalidOperationException($"[MacBuildScript] Refusing to delete unsafe build path: {buildPath}");
        ...
        private static void CopyVideoFiles(string buildPath, IReadOnlyCollection<string> videoFileNames)
        {
            ...
            {
                Debug.LogWarning("[MacBuildScript] No MiniGameData video files were found.");
                return;
            }
            ...
            var targetDir = Path.Combine(buildPath, StreamingAssetsRelativeDir);
            ...
            Debug.Log($"[MacBuildScript] Copied {copiedCount} video file(s) to {targetDir}.");
        }
        ...
        private static IReadOnlyCollection<string> GetReferencedVideoFileNames()
        {
            ...
            foreach (var guid in AssetDatabase.FindAssets("t:MiniGameData"))
            {
                ...
                if (!File.Exists(source))
                    throw new InvalidOperationException($"[MacBuildScript] Video file was not found for {assetPath}: {source}");
        ...
        private static string NormalizeVideoFileName(string fileName, string assetPath)
        {
            if (string.IsNullOrWhiteSpace(fileName))
                throw new InvalidOperationException($"[MacBuildScript] Video file name is empty: {assetPath}");

            fileName = fileName.Trim();
            if (fileName.IndexOfAny(new[] { '/', '\\' }) >= 0)
                throw new InvalidOperationException($"[MacBuildScript] Video file name must not contain a path: {assetPath} ({fileName})");
            ...
            if (!fileName.EndsWith(".mp4", StringComparison.OrdinalIgnoreCase))
                throw new InvalidOperationException($"[MacBuildScript] Video file must be an .mp4 file: {assetPath} ({fileName})");
        ...
        private static DirectoryInfo GetProjectRoot()
        {
            ...
            if (projectRoot == null)
                throw new InvalidOperationException("[MacBuildScript] Could not resolve Unity project root.");
